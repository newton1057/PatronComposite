...
public abstract class Composite {
    ...
    void DeleteAll(Lista A, int Index_Pos){
        if(A.Objetos.get(Index_Pos).Tipo=="Lista"){
            Lista aux = (Lista) A.Objetos.get(Index_Pos);
            if(aux.Validar_Nodos(aux)== false){
                A.Objetos.remove(Index_Pos);
            }
        }else{
                A.Objetos.remove(Index_Pos);
        }
